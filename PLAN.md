...
- **Codex 用量**：每課 ≈ 3.1 萬 tokens，ChatGPT Plus 的 5 小時額度一次約撐 8 課；一個階段 24 課要分 2–3 個窗口。做法：分批跑（Bryant 決定 2026-09-20），先生下週要用的課。

## 6c. 第 2 階段課綱設計備忘（2026-09-22，對照蓮先生電子書後定）

- **動詞分類先教**：第 2 階段第一課（W05D1）先教「看 ます形前一個假名判斷幾類：i 段＝一類、e 段＝二類、する／来る＝三類」，之後每個變形都用同一套三步：判斷幾類 → 這情境用什麼形 → 查規則表。App 加一頁「動詞 12 形規則表」（三類 × 12 形，自己寫，不用書的內容）
- **12 形的順序**跟 SPEC 階段對應：第 2 階段（動詞核心）教 ます形（已）、原形、ない形、可能形前半；第 3 階段（て形應用）て形、た形；第 4 階段（普通形）ない・た・原形的常體用法＋禁止／ば／命令／意向；受身・使役放 N4
- **助詞**：第 1 階段已覆蓋 は・に・を・と・へ・で・も・の・か；第 2 階段補 や（部分列舉）、が（存在句・好き・上手）；進階 7 個（轉折が、反射と、頻率に、原因から／ので／で）分散在第 3–4 階段
- **曝光**：生第 2 階段時 prompt 已有「必用複習字」；生完跑 `exposure.py --stage 1` 看第 1 階段的字有沒有被回收
- 延伸閱讀每週一篇繼續；W01 那篇（小孩視角）找機會重生
